	...
	if(currentcource == R_cource){
		...
		switch(currentstate){
			...
			case STRAIGHT1:
				...
				// linetrace.PIDSetRun(speed, 0.800, 0.800, 0.800, 1);
				linetrace.PIDSetRun(speed, 3, 3, 20, 1);
				//　左カーブは　0.450, 0.05, 0.120, 1 -> 最後のカーブ

				local.SetPoint();

				//現在のxとthetaの値を再セットする
				if(local.GetLen() >= 2000){//単位はmm
					//currentstate = 20;
				}                
				break;
				...
				// 	speed = speed - 1;
				// }
				// linetrace.PIDSetRun(speed, 0.150, 0.05, 0.300, 1);
...
*/
		...
		switch(currentstate){
			...
			case STRAIGHT_L1:
				...
				linetrace.PIDSetRun(35, 0.009, 0.1, 0.100, 1);
				//　左カーブは　0.450, 0.05, 0.120, 1 -> 最後のカーブ
			...
			case 20:
				...
				linetrace.PIDSetRun(30, 0.200, 0.05, 0.100, 1);
				currentstate = 20;
			    break;
